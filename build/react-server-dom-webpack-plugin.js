...
var clientFileName = require.resolve('react-server-dom-webpack');
...
class ReactFlightWebpackPlugin {
  ...
  apply(compiler) {
    ...
    compiler.hooks.emit.tap(PLUGIN_NAME, function (compilation) {
      var json = {};
      Array.from(compilation.chunkGroups).forEach(function (chunkGroup) {
        var chunkIds = Array.from(chunkGroup.chunks).map(function (c) {
          return c.id;
        });
        ...
        Array.from(chunkGroup.chunks).forEach(function (chunk) {
          const chunkModules = chunkGraph.getChunkModulesIterable(chunk)
          for (const mod of chunkModules) {
            const moduleId = chunkGraph.getModuleId(chunk)
            recordModule(moduleId, mod); // If this is a concatenation, register each child to the parent ID.

            if (mod.modules) {
              Array.from(mod.modules).forEach(function (concatenatedMod) {
      ...
      compilation.emitAsset(_this.manifestFilename, new sources.RawSource(output))
    });
